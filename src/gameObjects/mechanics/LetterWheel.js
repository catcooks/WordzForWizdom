...
export default class LetterWheel extends Phaser.GameObjects.Container {
    ...
    setupGlobalListeners() {
        ...
        // Scout Rule: Clean up listeners when this object is destroyed to prevent memory leaks/double-firing
        this.on('destroy', () => {
            this.scene.input.keyboard.off('keydown', this.handleKeyboardInput, this);
        });
    }

    // --- NEW: Keyboard Handling (SRP Applied) ---
    ...
    handleKeyboardInput(event) {
        ...
        if (key === 'ENTER') {
            ...
            this.swapLastLetter();
        } else if (/^[A-Z]$/.test(key)) { // KISS: Simple regex to check if it's a letter A-Z
            this.typeLetter(key);
        }
    }

    typeLetter(character) {
        // Find all unselected letters that match the typed character
        const availableSprites = this.letterSprites.filter(bg => {
            const letterChar = bg.parentContainer.char.toUpperCase();
            const isUnselected = !this.selectedSequence.includes(bg);
            
            return letterChar === character && isUnselected;
        });

        // If we found a match on the wheel, select the first one
        if (availableSprites.length > 0) {
            this.selectLetter(availableSprites[0]);
        }
    }
    ...
    swapLastLetter() {
        ...
        const lastSelectedBg = this.selectedSequence[this.selectedSequence.length - 1];
        const lastChar = lastSelectedBg.parentContainer.char.toUpperCase();

        // Get all sprites that match the character of the last selected letter
        const matchingSprites = this.letterSprites.filter(bg => 
            bg.parentContainer.char.toUpperCase() === lastChar
        );

        // If there's only one of this letter on the wheel, there's nothing to swap
        if (matchingSprites.length <= 1) return;

        const currentIndex = matchingSprites.indexOf(lastSelectedBg);

        // Loop through matches to find the next available identical letter
        for (let i = 1; i < matchingSprites.length; i++) {
            const nextIndex = (currentIndex + i) % matchingSprites.length;
            const candidateSprite = matchingSprites[nextIndex];

            // If this candidate isn't currently selected somewhere else in the sequence
            if (!this.selectedSequence.includes(candidateSprite)) {
                this.deselectLastLetter();
                this.selectLetter(candidateSprite);
                return;
            }
        }
    }
