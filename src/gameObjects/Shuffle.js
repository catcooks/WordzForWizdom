...
export default class Shuffle extends Phaser.GameObjects.Sprite {
    ...
    executeShuffle() {
        ...
        const count = this.wheel.letters.length; // Use the actual word length
        ...
        const effectsData = [
            { name: 'fire', color: 0xff4444 }, { name: 'heal', color: 0x44ff44 },
            { name: 'void', color: 0xaa44ff }, { name: 'ice', color: 0x4444ff }
        ];

        // 1. Shuffle the characters strings stored in the wheel
        ...
        // 3. Clear the wheel's sprite list to rebuild it
        ...
        oldLetterSprites.forEach((bg, i) => {
            ...
            const char = this.wheel.letters[i]; // Get the new char after shuffle
            ...
            this.scene.tweens.add({
                ...
                onComplete: () => {
                    ...
                    this.scene.tweens.add({
                        ...
                        ease: 'Back.easeOut',
                        onComplete: () => {
                            // Re-enable input exactly like your createWheel does
