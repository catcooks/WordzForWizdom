export default class StatBox extends Phaser.GameObjects.Container {
    ...
    updateHealth() {
        ...
        this.hearts.forEach((heart) => {
            if (tempHp >= 10) {
                heart.setFrame(2); // Full Heart
                tempHp -= 10;
            } else if (tempHp >= 5) {
                heart.setFrame(3); // Half Heart
                tempHp -= 5;
            } else {
                heart.setFrame(7); // Empty Heart
                tempHp = 0;
            }
        });
        ...
        this.shields.forEach((shieldIcon) => {
            if (tempShield >= 10) {
                shieldIcon.setFrame(4); // Full Shield
                shieldIcon.setAlpha(1).clearTint();
                tempShield -= 10;
            } else if (tempShield >= 5) {
                shieldIcon.setFrame(5); // Half/Cracked Shield
                shieldIcon.setAlpha(1).clearTint();
                tempShield -= 5;
            } else {
                shieldIcon.setFrame(6); // Empty Shield
    ...
    createShieldBoard(startXOffset, yOffset = 0, masterScale = 1) {
        ...
        for (let i = 0; i < totalShieldIcons; i++) {
            ...
            const shieldIcon = this.scene.add.sprite(x, y, 'icon-stats', 1)
                .setScale(iconScale);
    ...
    addShield(x, y, value) {
        let frame = 4; // Default Full Heart

        if (value === 5) {
            frame = 5; // Set this to your half-heart frame index
        } else if (value === 0) {
            frame = 6; // Set this to your empty heart frame index
        }

        const heart = this.scene.add.sprite(x, y, 'icon-stats', frame).setScale(this.iconScale);
        heart.initialX = x;
        heart.initialY = y;
    ...
    addHeart(x, y, value) {
        let frame = 2; // Default Full Heart

        if (value === 5) {
            frame = 3; // Set this to your half-heart frame index
        } else if (value === 0) {
            frame = 7; // Set this to your empty heart frame index
        }
